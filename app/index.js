...
function newTab (new_url = undefined, toggle = "active") {
  let webview_url = new_url != undefined ? new_url : def_url
  if (toggle == "active") {
    // 新建标签栏处于活动状态,隐藏其他标签
  ...
  // 增加标签
  $TabBar.append('<li style="display:none;" id="tb-' + tab_num + '" web-id="' + tab_num + '" class="web-label">' +
  ...
  $BrowserContainer.append('<div class="browser-content" style="display:none" id="wbc-' + tab_num + '">' +
    ...
    '</div><webview preload="preload.js" class="browser" id="wb-' + tab_num + '" web-id="' + tab_num + '" src="' + webview_url + '" webpreferences="nodeIntegration=true,contextIsolation=false, experimentalFeatures=true,webviewTag=true,enableRemoteModule=true"></webview>' +
    '<div class="show_url" id="su-' + tab_num + '"></div></div>')

  // 新版electron webview设置webPreferences,需要在元素标签上设置webpreferences
  ...
  if (toggle == "active") {
    // 新建标签栏处于活动状态,页面内容显示与标签定位到活动状态
    cur_tab = tab_num;
    cur_webview = document.querySelector("#wb-" + cur_tab);
    $('#tb-' + tab_num).show().addClass("selectd");
  ...
  $(".web-label-container").hover(function () { }, function () {
    $perview.hide(200);
  })
  ...
  webview.addEventListener('did-start-loading', (e) => {
    ...
    var tab = $TabBar.find("#tb-" + web_id)
    tab.find(".favicon").attr("src", "imgs/earth.png").css("border-radius", "50%");
  ...
  webview.addEventListener('did-stop-loading', (e) => {
    ...
    // }
    // new_url != undefined ? new_url : def_url
    var tab = $TabBar.find("#tb-" + web_id)
  ...
  webview.addEventListener('context-menu', function (event) {
    ...
    //     if (event.target.className.indexOf('noright') != -1) {
    //         return false
    //     }
    // }
    // 判断是否是导航栏上的列表右击
    ...
    //         // 导航栏右击要提供新标签页打开
    //         if (event.path[i].nodeName == 'A' && (event.path[i].href || event.path[i].hash)) {
    //             url = event.path[i].href ? event.path[i].href : ''
    //             hash = event.path[i].hash ? event.path[i].hash : ''
    //         }
    //     }
    // }
    // console.log(event)
    let data = {
      // 右击的标签名
      input_type: event.params.inputFieldType,
      // a标签的链接
      href: event.params.linkURL,
      // 一般为图片链接的地址
      src: event.params.srcURL,
      // 选中的文本
      media_type: event.params.mediaType,
      // 选中的文本
      txt: event.params.selectionText,
      // // 系统路径hash值
      // hash: hash
      // 当前webviewID
      // web_id::event.target.localName == 'webview' ? event.target.id : undefined,
    }
    ipcRenderer.send('show-contextmenu', data)

  });
  // webview.addEventListener('dragend', function(event) {
  //     // console.log(event)
  // });
...
// const winBrowser = BrowserWindow.getFocusedWindow().webContents;
...
// 获取窗口是否最大化
function get_win_isMaximized () {
  ipcRenderer.send('winBrowser_event', 'isMaximized')
  return new Promise(resolve => {
    // 监听返回
    ipcRenderer.on('win_isMaximized', function (event, arg) {
      // console.log('win_isMaximized')
      resolve(arg)
    });
  })
}

async function win_mize () {
  let isMaximized = await get_win_isMaximized()
  if (isMaximized) {
    windowMaximize();
...
ipcRenderer.on('window_on', function (event, arg) {
  // console.log('window_on')
  // console.log(arg)
  switch (arg) {
    case 'maximize':
      // 当窗口最大化时间调用
...
// 新标签打开
ipcRenderer.on('rendering_link_open', function (event, arg) {
  newTab(arg,)
});
// 刷新页面
ipcRenderer.on('reload_webview', function (event, arg) {
  if (arg == 'reloadIgnoringCache') {
    // 刷新访客页并忽略缓存
    cur_webview.reloadIgnoringCache()
  } else {
    // 刷新访客页
    cur_webview.reload()
  }
...
ipcRenderer.on('search_text', function (event, arg) {
  search_text()
});
...
for (var i = 0; i < btn_minis.length; i++) {
  btn_minis[i].addEventListener("click", function (e) {
    // winBrowser.minimize();
    ipcRenderer.send('winBrowser_event', 'minimize')
  });
}
// 最大化与退出最大化
for (var i = 0; i < btn_max.length; i++) {
  btn_max[i].addEventListener("click", async function (e) {
    let isMaximized = await get_win_isMaximized()
    if (isMaximized) {
      ipcRenderer.send('winBrowser_event', 'unmaximize')
    } else {
      ipcRenderer.send('winBrowser_event', 'maximize')
    }
  });
}
  ...
  data: [
...

    {
      ...
      "on": function () {
        ...
        ipcRenderer.send('winBrowser_event', 'isFullScreen')
        moreHide();
      },
      "reg": function () {
        // mousetrap.bind("F11", () => {
...
// 获取是否新窗口打开
let new_link = getValue(window.location.search, 'new_link')
// 获取是否有hash链接
let hash_link = getValue(window.location.search, 'hash_link')

// 新建标签

if (new_link) {
  // 打开指定链接
  var cur_view = newTab(hash_link ? new_link + '#' + hash_link : new_link, "active")
} else {
  // 打开默认链接
...
//         alert(selectText);
//     } }));
// var browser_card = document.querySelector('#card')
// /**
//  * 判断点击区域可编辑
//  */
